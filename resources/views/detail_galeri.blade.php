...
@section('title', $album->nama_album . ' - Galeri Sukorame')
...
@section('content')
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
                @foreach ($fotos as $index => $foto)
                    <div class="bg-white rounded-2xl shadow outline outline-2 outline-[#d4f78f] overflow-hidden">
                        <img src="{{ asset('storage/' . $foto->foto_path) }}" alt="Foto {{ $loop->iteration }}"
                            class="w-full h-64 object-cover cursor-pointer" onclick="openLightbox({{ $index }})">
                    </div>
                @endforeach
            </div>
        </div>
                ...
                <div class="grid md:grid-cols-3 gap-8">
                    @foreach($albumLain as $lain)
                        <a href="{{ route('publik.galeri_detail', $lain->id_galeri) }}"
                            ...
                            <img src="{{ asset('storage/' . $lain->cover_path) }}" alt="{{ $lain->nama_album }}"
    ...
    <div id="lightbox" class="fixed inset-0 bg-black bg-opacity-90 hidden items-center justify-center z-50">
        <button id="closeBtn" class="absolute top-5 right-5 text-white text-3xl font-bold focus:outline-none"
            onclick="closeLightbox()">&times;</button>

        <button id="prevBtn" class="absolute left-5 text-white text-4xl font-bold px-3 focus:outline-none"
        ...
        <button id="nextBtn" class="absolute right-5 text-white text-4xl font-bold px-3 focus:outline-none"
