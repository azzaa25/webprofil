...
@section('title', $berita->judul . ' - Berita Kelurahan Sukorame')
...
<section class="py-12 bg-[#eef7ec] border-t border-gray-200">
    <div class="container mx-auto px-4 max-w-5xl">
        <h2 class="text-2xl font-bold text-gray-900 mb-6 text-center">Berita Lainnya</h2>
        <div class="grid md:grid-cols-3 gap-8">
            @foreach($beritaLain as $lain)
                <div class="bg-white rounded-2xl shadow hover:shadow-lg overflow-hidden transition-transform hover:-translate-y-1">
                    <img src="{{ asset('storage/' . $lain->gambar) }}" alt="{{ $lain->judul }}" class="w-full h-48 object-cover">
                    <div class="p-5">
                        <h3 class="text-lg font-semibold text-gray-900 mb-2 hover:text-[#4c3588] transition-colors">
                            <a href="{{ route('publik.berita_detail', $lain->slug) }}">{{ $lain->judul }}</a>
                        </h3>
                        <p class="text-sm text-gray-600 mb-2">
                            {{ \Carbon\Carbon::parse($lain->tanggal_publikasi)->translatedFormat('d F Y') }}
                        </p>
                        <p class="text-gray-700 text-sm">{{ Str::limit(strip_tags($lain->konten), 100, '...') }}</p>
                        <a href="{{ route('publik.berita_detail', $lain->slug) }}" class="text-[#4c3588] font-semibold text-sm hover:underline mt-2 inline-block">Baca Selengkapnya →</a>
                    </div>
                </div>
            @endforeach
        </div>
